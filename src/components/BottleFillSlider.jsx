...
function BottleFillSlider({ image, name, fillLevel, onFillLevelChange, onPrev, onNext }) {
  ...
  const imageSource = useMemo(() => {
    ...
    return resolved || (image && (image.startsWith('http') || image.startsWith('file') ? { uri: image } : null));
  }, [name, image]);
    ...
    <View style={styles.container}>
      <View style={styles.row}>
        {showNav ? (
          ...
          <View style={[styles.chevronSide, { height: bottleHeight }]} />
        )}
        ...
        <View style={[styles.bottleWrap, { height: bottleHeight }]}>
          {imageSource ? (
            ...
            <View style={styles.placeholderBottle}>
              <Icon name={Icons.localBar} size={48} color={colors.textSecondary} style={styles.placeholderIcon} />
            </View>
          )}
          <Animated.View
            style={[styles.fillOverlay, { height: overlayHeight }]}
            pointerEvents="none"
          >
            <Svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none" style={StyleSheet.absoluteFill}>
              <Defs>
                <LinearGradient id="bottleFillGrad" x1="0" y1="0" x2="0" y2="1">
                  <Stop offset="0" stopColor={FILL_GRADIENT.lightBlue} stopOpacity={0.85} />
                  <Stop offset="1" stopColor={FILL_GRADIENT.darkBlue} stopOpacity={0.9} />
                </LinearGradient>
              </Defs>
              <Rect x={0} y={0} width={100} height={100} fill="url(#bottleFillGrad)" />
            </Svg>
          </Animated.View>
          <Animated.View
            {...pan.panHandlers}
