...
export default function EditProductScreen({ route, navigation }) {
  ...
  const { updateProduct, currentCategoryName, dbReady } = useInventory();
  const { t } = useLanguage();
  ...
  }, [product?.id, name, volume, subCategory, price, imageUri, imageUrl, updateProduct, navigation, validate]);
    ...
    <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          <Text style={styles.areaHint}>{currentCategoryName}</Text>
          ...
          <View style={styles.field}>
            ...
            <TouchableOpacity style={styles.imageTouchable} onPress={handlePickImage} activeOpacity={0.8}>
                {(imageUri || imageUrl.trim()) ? (
                  <View style={styles.imagePreviewWrap}>
                    <Image source={{ uri: imageUri || imageUrl.trim() }} style={styles.imagePreview} resizeMode="cover" />
            ...
            <TextInput style={styles.input} value={imageUrl} onChangeText={setImageUrl} placeholder={t('imageUrlPlaceholder')} placeholderTextColor={colors.textSecondary} autoCapitalize="none" keyboardType="url" />
          </View>

          <View style={styles.field}>
            <Text style={styles.label}>{t('productName')} *</Text>
            <TextInput style={[styles.input, errors.name && styles.inputError]} value={name} onChangeText={(v) => { setName(v); setErrors((e) => ({ ...e, name: undefined })); }} placeholder={t('productNamePlaceholder')} placeholderTextColor={colors.textSecondary} autoCapitalize="words" />
            {errors.name ? <Text style={styles.errorText}>{errors.name}</Text> : null}
          </View>

          <View style={styles.field}>
            <Text style={styles.label}>{t('volumeMl')}</Text>
            <TextInput style={[styles.input, errors.volume && styles.inputError]} value={volume} onChangeText={(v) => { setVolume(v.replace(/[^0-9]/g, '')); setErrors((e) => ({ ...e, volume: undefined })); }} placeholder={t('volumePlaceholder')} placeholderTextColor={colors.textSecondary} keyboardType="number-pad" />
            {errors.volume ? <Text style={styles.errorText}>{errors.volume}</Text> : null}
          </View>

          <View style={styles.field}>
            <Text style={styles.label}>{t('category')}</Text>
            <TextInput style={styles.input} value={subCategory} onChangeText={setSubCategory} placeholder={t('categoryPlaceholder')} placeholderTextColor={colors.textSecondary} autoCapitalize="words" />
          </View>

          <View style={styles.field}>
            <Text style={styles.label}>{t('purchasePrice')}</Text>
            <TextInput style={[styles.input, errors.price && styles.inputError]} value={price} onChangeText={(v) => { setPrice(v); setErrors((e) => ({ ...e, price: undefined })); }} placeholder={t('purchasePricePlaceholder')} placeholderTextColor={colors.textSecondary} keyboardType="decimal-pad" />
...
const styles = StyleSheet.create({
  ...
  imagePreviewWrap: { width: IMAGE_PREVIEW_SIZE, height: IMAGE_PREVIEW_SIZE, borderRadius: borderRadius.md, overflow: 'hidden', position: 'relative' },
